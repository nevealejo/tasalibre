...
<script>
  ...
  const callApi = async (skipContact = false) => {
    ...
    try {
      // BLOQUE 21: fix de colisión de nombres — muchas calles argentinas se
      // llaman igual que un partido/localidad conocida (el caso real que
      // falló: "Moreno" es la calle en Quilmes pedida, pero también es el
      // nombre de un partido entero de GBA Oeste; sin más contexto, las
      // búsquedas devolvían mayormente inmuebles del PARTIDO de Moreno en vez
      // de la calle Moreno en Quilmes — la propia IA lo diagnosticó así en el
      // análisis). Al incluir el número de puerta junto al nombre de la calle
      // entre comillas ("Moreno 450" en vez de Moreno suelto), la búsqueda
      // queda anclada a una frase mucho más específica que rara vez aparece
      // en contenido sobre la localidad homónima.
      const calleRef = calle ? '"' + calle + (numero ? " " + numero : "") + '" ' : "";
      ...
      const prompt = "Sos tasador inmobiliario Argentina. CONSERVADOR y PRECISO.\n" +
        ...
        (esCerradoSearch && (casaNombreBarrio||nombreBarrioPrivado)
          ...
          : "REGLAS: 1)BASE DE CALCULO: precio_base_m2_usd es el promedio de precio/m2 (sobre m2 CUBIERTOS) de los comparables reales encontrados en la busqueda para " + address + " y su zona inmediata — NUNCA un valor de memoria, tope regional fijo, ni comparacion con otras zonas (CABA u otras). 2)Priorizar los 6 comparables MAS CERCANOS a " + address + " por sobre cualquier otro. 2b)CRITICO NOMBRE DE CALLE: la calle \"" + (calle||"") + "\" puede coincidir con el nombre de un partido/localidad conocida de Argentina (ej: Moreno, Merlo, Tigre, San Martin son calles MUY comunes que tambien son nombres de partidos enteros). La propiedad tasada esta en la localidad \"" + barrio + "\" — descarta cualquier comparable que en realidad pertenezca a la localidad/partido homonima de la calle en vez de a \"" + barrio + "\", aunque el texto de busqueda lo haya traido. Si la mayoria de resultados encontrados son de esa localidad homonima y no de \"" + barrio + "\", tratalos como comparables NO utilizables (no los promedies) y aclaralo en el analisis. 3)Si el contexto incluye una 'ANCLA CALCULADA POR CODIGO', tu precio_base_m2_usd debe estar dentro de +-20% de esa cifra, salvo justificacion explicita en el analisis. 4)Rango+-5%. 5)CONSERVADOR: ante pocos comparables, preferir un valor mas bajo antes que sobreestimar. 6)NO DEJES 'comparables' VACIO SI HAY DATOS: si tu propia busqueda o el contexto 'COMPARABLES' de mas arriba mencionan aunque sea 1 o 2 publicaciones reales con direccion+precio+m2, INCLUILAS en el campo 'comparables' del JSON final — no las omitas ni devuelvas el array vacio solo porque son pocas; pocas publicaciones reales son mejores que ninguna. Recien devolvé 'comparables' vacio si genuinamente no encontraste NINGUNA publicacion con datos suficientes. 6b)PROHIBIDO INVENTAR: cada 'direccion' del JSON final tiene que ser una calle+altura REAL citada en los resultados de busqueda (ej 'Alberdi 234'). Tenes PROHIBIDO usar etiquetas genericas o de relleno como 'Comparable zona X', 'Propiedad similar', 'Referencia N' ni ninguna variante inventada para completar el array — eso es peor que dejarlo mas corto, porque aparenta ser una publicacion real sin serlo. Si solo tenes 1 o 2 publicaciones reales, devolvé solo esas." + (streetsNearby && streetsNearby.length
        ...
        (msgContent.length === 0
          ? "REGLA CRITICA - SIN FOTOS: no se adjunto NINGUNA foto en esta tasacion. Tenes PROHIBIDO afirmar en 'analisis' o en cualquier campo que hayas visto, analizado o evaluado fotos, materiales, terminaciones, humedad, grietas, pisos ni ningun otro detalle visual — eso seria inventado. Basa el estado unicamente en lo que el propietario declaro en el formulario (campo 'est' de PROPIEDAD). Para 'clasificacion', usa siempre 'normal' en los tres campos y 'necesita_refaccion_integral':false, sin justificarlo con supuesta evidencia visual.\n"
          : "") +
        "\n" +
        "CLASIFICACION (categorias para el sistema" + (msgContent.length>0 ? " — a partir de las fotos" : " — sin fotos disponibles, usar 'normal' siempre segun la regla de arriba") + "; NO calcules ningun porcentaje, solo elegi la categoria):\n" +
      ...
      if (!esCerradoSearch && Array.isArray(parsed.comparables)) {
        ...
        // BLOQUE 27b: la IA a veces inventa una etiqueta genérica en vez de
        // citar una dirección real (visto en vivo: "Comparable zona Quilmes
        // 1", "Comparable zona Quilmes 2") — y como el geocoder igual
        // resuelve esas frases a ALGÚN punto de la localidad, la distancia
        // "da bien" y el comparable pasa el filtro pareciendo verificado
        // aunque no corresponda a ninguna publicación real. Se descarta
        // cualquier "dirección" que sea claramente una etiqueta genérica
        // (arranca con "comparable/propiedad/inmueble/publicación/aviso") o
        // que no tenga una numeración de calle real (2+ dígitos) — mismo
        // criterio que ya se usa para las URLs individuales en BLOQUE 26c.
        const pareceDireccionReal = (direccion) => {
          if (!direccion) return false;
          const d = direccion.trim();
          if (/^(comparable|propiedad|inmueble|publicaci[oó]n|aviso|resultado|opci[oó]n|referencia)\b/i.test(d)) return false;
          return /\d{2,}/.test(d);
        };
        const totalAntesDireccionReal = parsed.comparables.length;
        parsed.comparables = parsed.comparables.filter(c => pareceDireccionReal(c.direccion));
        if (parsed.comparables.length < totalAntesDireccionReal) {
          console.log(`[COMPS] descartados ${totalAntesDireccionReal - parsed.comparables.length} comparables con dirección no verificable (etiqueta genérica, sin numeración real)`);
        }
        if (!coordsPropiedad) {
